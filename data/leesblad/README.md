...
## iPad/iPadOS TTS silently doing nothing (fixed 2026-09-22)
Chrome on iPad/iOS runs on WKWebView — Apple's Safari speech engine, not Chromium's — and that engine drops `speechSynthesis.speak()` calls made too long after the user's tap, with **no error at all**. `readWord()`'s "letter voor letter" mode plays 2-3 real-voice clips (~1-1.3s each) before the whole-word TTS call, several seconds past that window on iPad, even though the exact same code works fine on desktop Chrome. Fixed with `queueUtterance()` in `leesbladscript.js`: `speechSynthesis.speak()` is called *immediately* when the word is tapped (satisfying the gesture requirement) but the engine is put in `.pause()` right after, and only `.resume()`d once the letter breakdown finishes — the utterance was already accepted, so it plays normally at that point. `utter()` also calls `speechSynthesis.resume()` first (iOS sometimes leaves the engine stuck paused after the tab is backgrounded), and there's a one-time silent unlock utterance fired on the page's very first touch. Verified with a mocked `speechSynthesis` that `speak()`+`pause()` fire synchronously before any audio clip plays, `resume()` fires only after, and interrupting mid-word (tapping a different word before the first one's clips finish) cleanly cancels the pending utterance with no hang — could not verify on real iPad hardware, only via this mocked headless test.
