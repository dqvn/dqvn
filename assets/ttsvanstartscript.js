...
function reloadTable(jsonData) {
  ...
  jsonData.forEach((word, index) => {
    ...
    row.innerHTML = `
    <td>${index + 1}</td>
    <td onclick="speakText('${word.dutch?.replace(/'/g, "")}')"><span class="dutch-word" data-index="${index}">${word.dutch}</span><br/><span style="font-size: 0.5em;">/${word.pronunciation?.ipa || ""}/ <br/> /${word.pronunciation?.phonetic || ""}/</span></td>
