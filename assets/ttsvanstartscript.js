...
function reloadTable(jsonData) {
  ...
  jsonData.forEach((word, index) => {
    ...
    <td onclick="speakText('${word.dutchsentence?.replace(/'/g, "")}')"><span>/${word.pronunciation?.ipa || ""} ~ ${word.pronunciation?.phonetic || ""} <br/> ${word.dutchsentence}</span><br/><span class="hide-text" style="color: #3f3838ff; opacity: 0.3;">${word.englishtranslate}</span></td>
